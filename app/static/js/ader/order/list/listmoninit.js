$( function() {
	...
	let dateRow = function(timestamp) {
		...
		let ident = timestamp;
		let elem;
		elem = '<div class="row bg-secondary mt-2 py-2">';
			...
			elem += '</div>';
			elem += '<div class="col-2 text-center">';
	...
	let lastdayRow = function(lastTimestamp) {
		...
		let elem;
		elem = '<div class="row bg-secondary mt-2 py-2 lastday" id="lastday_'+ident+'">';
			elem += '<div class="col-1">';
	...
	let getOrders = function(timestamp) {
		...
		let ident = timestamp;
		let groupCode = $("#selWsgpId").val();
		$.ajax({
			type: 'GET',
			url: '/adOrdersAjax?begin=' + timestamp + '&selWsgpId='+groupCode
		})
	...
	$(".container").on('click', '.lastday', function(e) {
